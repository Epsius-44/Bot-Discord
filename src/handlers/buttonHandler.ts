...
export default new Handler({
  ...
  async execute(client: Client, files: string[]): Promise<void> {
    client.logger.info("button - Début du chargement des boutons");
    for (const file of files) {
      const button: Button = (await import(`${this.folder}/${file}`))
        .default as Button;
      client.appButtons.set(button.name, button);
      client.logger.debug(`button - Le bouton ${button.name} est chargé`);
    }
    client.logger.info("button - Fin du chargement des boutons");
  }
});
