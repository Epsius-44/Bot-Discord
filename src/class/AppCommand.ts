            ...
            this.execute = async (interaction: ChatInputCommandInteraction) => {
                ...
                if (!commandName) {
                    ...
                    interaction.client.logger.warn(
                        `[WARN] - L'utilisateur ${interaction.user.tag} a tenté d'exécuter une commande mais n'a pas fourni de sous-commande.`
                    )
                } else {
                    try {
                        const command = (
                        ...
                        interaction.client.logger.error(
                            `[ERROR] - Une erreur s'est produite lors de l'exécution de la commande ${this.data.name} ${
                                subCommandGroup ? `${subCommandGroup}/` : ''
            ...
            this.autocomplete = async (
                ...
                if (subCommandGroup || subCommandName) {
                    try {
                        ...
                        interaction.client.logger.error(
                            `[ERROR] - Une erreur s'est produite lors de l'autocomplétion de ${this.data.name} ${subCommandGroup ? `${subCommandGroup}/` : ''} ${subCommandName} pour ${interaction.user.tag}: ${error}`
                        )
                    }
                }
            }
