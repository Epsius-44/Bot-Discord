...
fn set_instance_data(mut cx: FunctionContext) -> JsResult<JsUndefined> {
    let redis_uri = cx.argument::<JsString>(0)?;
    let instance_id = cx.argument::<JsString>(1)?;
    let instance_name = cx.argument::<JsString>(2)?;
    let instance_version = cx.argument::<JsString>(3)?;
    let node_version = cx.argument::<JsString>(4)?;
    let client = redis::Client::open(redis_uri.value(&mut cx)).unwrap();
    ...
    con.hset_multiple::<String, &str, String, String>(
        format!("instances:{}", instance_id.value(&mut cx)),
        &[
...
fn lookup(mut cx: FunctionContext) -> JsResult<JsBoolean> {
    ...
    let mut keys: Vec<String> = con.keys("online:lists:*").unwrap();
    keys.sort();
    match keys.len() {
        0 => {
            let _ = con.sadd::<&str, String, String>("online:lists:0", instance_id.value(&mut cx));
            let _ = con.sadd::<&str, String, String>("online:lists:1", instance_id.value(&mut cx));
            let _ = con.expire::<&str, i64>("online:lists:0", ONLINE_LISTS_TTL * 1);
            let _ = con.expire::<&str, i64>("online:lists:1", ONLINE_LISTS_TTL * 2);
        },
        1 => {
            let _ = con.sadd::<String, String, String>(keys[0].clone(), instance_id.value(&mut cx));
            let actual_ttl = con.ttl::<String, i64>(keys[0].clone());
            let next_name = keys[0].clone().replace("online:lists:", "").parse::<i64>().unwrap() + 1;
            let _ = con.sadd::<String, String, String>(format!("online:lists:{}", next_name), instance_id.value(&mut cx));
            let _ = con.expire::<String, i64>(format!("online:lists:{}", next_name), actual_ttl.unwrap() + ONLINE_LISTS_TTL);
        },
        _ => {
    ...
    let master_exist = con.exists::<&str,bool>("online:master").unwrap();
    if master_exist {
        let master_id = con.get::<&str, String>("online:master").unwrap();
        let master_in_list = con.sismember::<String, String, bool>(keys[0].clone(), master_id.clone()).unwrap();
        if !master_in_list {
            let _ = con.set::<&str, String, String>("online:master", instance_id.value(&mut cx));
            let _ = con.expire::<&str, i64>("online:master", ONLINE_LISTS_TTL * 1);
...
fn get_cluster_info(mut cx: FunctionContext) -> JsResult<JsArray> {
    ...
    let client = redis::Client::open(redis_uri.value(&mut cx)).unwrap();
    let mut con = client.get_connection_with_timeout(std::time::Duration::from_secs(5)).unwrap();
    let keys: Vec<String> = con.keys("instances:*").unwrap();
    let master_id = con.get::<&str, String>("online:master").unwrap();
    let cluster_info = cx.empty_array();
    for key in keys {
        let id = key.clone().replace("instances:", "");
        ...
        let mut online_keys: Vec<String> = con.keys("online:lists:*").unwrap();
        online_keys.sort();
